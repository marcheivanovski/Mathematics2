...
n <- 2
m <- 10000
samples <- metropolis_hastings(p_scenario1, m, c(0.2, 0.5), diag(2), 2)
...
#g1 <- ggplot(df_samples, aes(x = x, y = y)) + 
#  geom_line()
#plot(g1)
...
```{r}
hamiltonian_sampling <- function (L, epsilon, current_q, m, minus_logf, minus_logf_grad){
  samples <- NULL
  for (i in 1:m) {
    #print(i)
...
minus_logf_grad <- function(x) {
  ...
  for (i in 1:nrow(df)){
    ...
    if (df[i,]$y==1){
      ...
      same_part <- (1-1/(1+exponent))**(-1)*(1+exponent)**(-2)*exponent*(-1)
      sum_beta1 <- sum_beta1 + same_part*df[i,1]
      sum_beta2 <- sum_beta2 + same_part*df[i,2]
    }
  }
  #model  <- glm(y ~ X2, family="binomial", data=df)
  #y_pred <- predict(model, data.frame(X2=df[,2]), type="response")
  return (c(-1*sum_beta1, -1*sum_beta2))
}

## HMC
L = 5
epsilon = 0.1
current_q = c(0.58,-0.15)
m = 10
